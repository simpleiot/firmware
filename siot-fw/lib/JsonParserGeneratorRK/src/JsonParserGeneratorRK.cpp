...
JsonBuffer::JsonBuffer(char* buffer, size_t bufferLen)
    : buffer(buffer)
    , bufferLen(bufferLen)
    , offset(0)
    , staticBuffers(true)
...
void JsonBuffer::clear() { offset = 0; }
...
JsonParser::JsonParser(char* buffer, size_t bufferLen,
    JsonParserGeneratorRK::jsmntok_t* tokens,
    size_t maxTokens)
    : JsonBuffer(buffer, bufferLen)
    , tokens(tokens)
    , maxTokens(maxTokens)
{
}
...
bool JsonParser::allocateTokens(size_t maxTokens)
{
    if (!staticBuffers) {
        ...
        if (tokens) {
            newTokens = (JsonParserGeneratorRK::jsmntok_t*)realloc(
                buffer, sizeof(JsonParserGeneratorRK::jsmntok_t) * maxTokens);
        } else {
            newTokens = (JsonParserGeneratorRK::jsmntok_t*)malloc(
                sizeof(JsonParserGeneratorRK::jsmntok_t) * maxTokens);
        }
        if (newTokens) {
            tokens = newTokens;
...
bool JsonParser::parse()
{
    if (tokens) {
        ...
        int result = JsonParserGeneratorRK::jsmn_parse(&parser, buffer, offset,
            tokens, maxTokens);
    ...
    // If we get here, tokens will always be == 0; it would have been freed if it
    // was too small, and this code is never executed for staticBuffers == true

    maxTokens = (size_t)result;
    if (maxTokens > 0) {
        tokens = (JsonParserGeneratorRK::jsmntok_t*)malloc(
            sizeof(JsonParserGeneratorRK::jsmntok_t) * maxTokens);
    ...
    /*
  for(const JsonParserGeneratorRK::jsmntok_t *token = tokens; token < tokensEnd;
  token++) { printf("%d, %d, %d, %d\n", token->type, token->start, token->end,
  token->size);
  }
  */
...
{
    for (const JsonParserGeneratorRK::jsmntok_t* token = tokens;
         token < tokensEnd; token++) {
...
JsonParser::getTokenByIndex(const JsonParserGeneratorRK::jsmntok_t* container,
...
bool JsonParser::getKeyValueTokenByIndex(
    ...
    const JsonParserGeneratorRK::jsmntok_t*& value, size_t desiredIndex) const
{
...
bool JsonParser::getValueTokenByKey(
    const JsonParserGeneratorRK::jsmntok_t* container, const char* name,
    const JsonParserGeneratorRK::jsmntok_t*& value) const
{
    ...
    for (size_t ii = 0; getKeyValueTokenByIndex(container, key, value, ii);
         ii++) {
...
bool JsonParser::getValueTokenByIndex(
    const JsonParserGeneratorRK::jsmntok_t* container, size_t desiredIndex,
    const JsonParserGeneratorRK::jsmntok_t*& value) const
{
    size_t index = 0;
    const JsonParserGeneratorRK::jsmntok_t* token = container + 1;
...
bool JsonParser::getValueTokenByColRow(
    const JsonParserGeneratorRK::jsmntok_t* container, size_t col, size_t row,
    const JsonParserGeneratorRK::jsmntok_t*& value) const
{
    ...
    char* dst, size_t dstLen) const
{
    ...
    int ii;
    for (ii = 0; ii < (token->end - token->start) && ii < ((int)dstLen - 1);
         ii++) {
...
bool JsonParser::getTokenJsonString(
    const JsonParserGeneratorRK::jsmntok_t* token, String& result) const
{
    result = "";
...
bool JsonParser::getTokenJsonString(
    const JsonParserGeneratorRK::jsmntok_t* token, char* str,
    size_t& bufLen) const
{
    JsonParserString strWrapper(str, bufLen);
...
JsonParserString::JsonParserString(char* buf, size_t bufLen)
    : str(0)
    , buf(buf)
    , bufLen(bufLen)
    , length(0)
...
JsonWriter::JsonWriter(char* buffer, size_t bufferLen)
    : JsonBuffer(buffer, bufferLen)
    , floatPlaces(-1)
{
    init();
...
void JsonWriter::insertString(const char* s, bool quoted)
{
    ...
    for (size_t ii = 0; s[ii] && offset < bufferLen; ii++) {
        if (s[ii] & 0x80) {
            ...
            } else if (((s[ii] & 0b11100000) == 0b11000000) && ((s[ii + 1] & 0b11000000) == 0b10000000)) {
                // 2-byte
...
JsonModifier::JsonModifier(JsonParser& jp)
    : jp(jp)
{
}
...
bool JsonModifier::removeKeyValue(
    const JsonParserGeneratorRK::jsmntok_t* container, const char* key)
{
...
bool JsonModifier::removeArrayIndex(
    const JsonParserGeneratorRK::jsmntok_t* container, size_t index)
{
...
bool JsonModifier::startModify(const JsonParserGeneratorRK::jsmntok_t* token)
{
    ...
    // origAfter, saveLoc, saveLoc - start);
...
void JsonModifier::finish()
{
    ...
    if (origAfter > 0) {
        memmove(jp.getBuffer() + start + getOffset(), jp.getBuffer() + saveLoc,
            origAfter);
    }
