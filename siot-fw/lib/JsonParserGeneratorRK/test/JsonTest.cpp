...
void _assertJsonParserBuffer(JsonParser& jp, const char* expected,
    size_t line)
{
...
#define assertJsonParserBuffer(jp, expected) \
    _assertJsonParserBuffer(jp, expected, __LINE__)

void _assertJsonWriterBuffer(JsonWriter& jw, const char* expected,
    size_t line)
{
...
#define assertJsonWriterBuffer(jw, expected) \
    _assertJsonWriterBuffer(jw, expected, __LINE__)

// assert(jw.getOffset() == 14 && strncmp(jw.getBuffer(),
// "\\b\\f\\n\\r\\t\\\"\\", 8) == 0);
...
int main(int argc, char* argv[])
{
    ...
    snow","icon":"13n"}],"clouds":{"all":88},"wind":{"speed":4.93,"deg":278.007},"snow":{"3h":0.22125},"sys":{"pod":"n"},"dt_txt":"2016-11-12
    21:00:00"},
     ...
     */
        ...
        // This is the last array element to make sure we've parsed correctly all
        // the way to to the end
        // {"dt":1479405600,"main":{"temp":267.329,"temp_min":267.329,"temp_max":267.329,"pressure":1024.37,"sea_level":1045.84,"grnd_level":1024.37,"humidity":91,"temp_kf":0},"weather":[{"id":600,"main":"Snow","description":"light
        // snow","icon":"13n"}],"clouds":{"all":80},"wind":{"speed":4.95,"deg":179.502},"snow":{"3h":0.105},"sys":{"pod":"n"},"dt_txt":"2016-11-17
        // 18:00:00"}]}
        assert(jp.getValueTokenByIndex(listToken, jp.getArraySize(listToken) - 1,
            listObj));
    ...
    // {"A--":{"M":{"M":2,"U":5000,"T":10,"C":[0,255]}}}
    {
        ...
        for (size_t ii = 0;; ii++) {
            ...
            if (!jp.getKeyValueTokenByIndex(jp.getOuterObject(), keyToken, valueToken,
                    ii)) {
                // Reached end
                assert(ii == 1);
                break;
            ...
            for (size_t jj = 0;; jj++) {
                ...
                if (!jp.getKeyValueTokenByIndex(valueTokenInnerM, keyTokenInner,
                        valueTokenInner, jj)) {
                    // Reached end
                    break;
                }
    ...
    // Writer test - nested
    {
        ...
        assertJsonWriterBuffer(
            jw, "{\"a\":[123,456,789],\"b\":{\"ba\":true,\"bb\":1234}}");
    }
    ...
    // Writer test - array of objects - low level API
    {
        ...
        for (int ii = 0; ii < 5; ii++) {
            // This used to be necessary, but now startObject takes care of this
            // automatically jw.insertCheckSeparator();
            jw.startObject();
    ...
    // Note: uses low-level API, probably best to use insertOrUpdateKeyValue in
    // most code
    {
        JsonParserStatic<512, 32> jp;
    ...
    // Append an array to an object
    {
        ...
        assertJsonParserBuffer(jp,
            "{\"a\":[1,2,3],\"b\":[\"test\"],\"c\":\"xxx\"}");
...
void printTokens(JsonParser& jp)
{
    ...
    for (JsonParserGeneratorRK::jsmntok_t* tok = jp.getTokens(); tok < tokensEnd;
         tok++) {
        printToken(jp, tok);
    }
}
...
void printJsonInner(JsonParser& jp,
...
void printJson(JsonParser& jp) { printJsonInner(jp, jp.getOuterToken(), 0); }
