...
protected:
    system_tick_t _timeout; // number of milliseconds to wait for the next char
...
public:
    ...
    void
    setTimeout(system_tick_t timeout); // sets maximum milliseconds to wait for
        // stream data, default is 1 second

    bool find(char* target); // reads data from the stream until the target string
        // is found
    ...
    bool findUntil(char* target,
        char* terminator); // as find but search ends if the terminator
    // string is found
